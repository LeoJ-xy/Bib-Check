...
class FixPlanner:
    ...
    def build_plan(self, entry: dict, issues: List[dict], online_data: dict) -> Dict:
        ...
        # Normalize arXiv DOI values first.
        arxiv_action = self._plan_arxiv_doi(entry)
        if arxiv_action:
            actions.append(arxiv_action)

        # Move arXiv venues out of journal/booktitle and into howpublished.
        ...
        # Normalize pages unless an online action already replaces them.
            ...
            if normalized_pages and normalized_pages != entry.get("pages"):
                actions.append(
                    self._make_action(
                        ...
                        reason="Normalize pages to BibTeX double-hyphen ranges and remove prefixes",
                    )
                )
    ...
    def _plan_arxiv_doi(self, entry: dict) -> Optional[dict]:
        """Normalize arXiv DOI casing; infer a DOI from eprint/url when possible."""
        doi = entry.get("doi")
        ...
        if doi and re.match(r"10\.48550/ARXIV\.", doi, flags=re.I):
            ...
            if new_doi != doi:
                return self._make_action(entry, "doi", new=new_doi, confidence=0.95, source="normalize", reason="Normalize arXiv DOI casing")
        ...
        if not doi:
            ...
            if arxiv_id:
                ...
                return self._make_action(entry, "doi", new=new_doi, confidence=0.9, source="arxiv", reason="Infer arXiv DOI from eprint/url")
        return None

    def _plan_arxiv_venue_migration(self, entry: dict) -> Optional[dict]:
        """Move arXiv journal/booktitle values to howpublished and remove the originals."""
        ...
        if current_venue and "arxiv" in current_venue.lower():
            # Create a migration action that writes howpublished and removes journal/booktitle.
            return self._make_action(
                ...
                confidence=0.95,
                source="normalize",
                reason="Move arXiv venue to howpublished",
    ...
    def _plan_from_resolved(self, entry: dict, resolved: dict, title_score: int) -> List[dict]:
        ...
        if resolved.get("doi"):
            ...
            if doi_new and doi_new != normalize_doi_value(entry.get("doi")):
                actions.append(self._make_action(entry, "doi", new=doi_new, confidence=conf, source=resolved.get("source", "online"), reason="DOI from authoritative source"))

        # Title
        if resolved.get("title"):
            if normalize_title(resolved["title"]) and normalize_title(resolved["title"]) != normalize_title(entry.get("title", "")):
                actions.append(self._make_action(entry, "title", new=resolved["title"], confidence=conf, source=resolved.get("source", "online"), reason="Synchronize title from authoritative source"))
        ...
        if resolved.get("authors"):
            ...
            if formatted_authors and normalize_title(entry.get("author", "")) != normalize_title(formatted_authors):
                actions.append(self._make_action(entry, "author", new=formatted_authors, confidence=conf, source=resolved.get("source", "online"), reason="Synchronize authors from authoritative source"))

        # Year
        if resolved.get("year"):
            if resolved["year"] != entry.get("year"):
                actions.append(self._make_action(entry, "year", new=str(resolved["year"]), confidence=conf, source=resolved.get("source", "online"), reason="Synchronize year from authoritative source"))
        ...
        if venue:
            ...
            if venue != current_venue:
                # For arXiv entries, use howpublished rather than journal.
                ...
                if is_arxiv:
                    ...
                    new_venue = venue
                    extra = None
                actions.append(self._make_action(entry, field, new=new_venue, confidence=conf, source=resolved.get("source", "online"), reason="Synchronize venue", extra=extra))
        ...
        for f in ["volume", "number", "pages"]:
            if resolved.get(f):
                ...
                if new_val and new_val != entry.get(f):
                    actions.append(self._make_action(entry, f, new=new_val, confidence=conf, source=resolved.get("source", "online"), reason=f"Fill missing {f}"))
        return actions
    ...
    def _plan_from_candidates(self, entry: dict, candidates: List[dict]) -> List[dict]:
        ...
        if best.get("doi") and conf >= self.config.mid_threshold:
            ...
            if doi_new and doi_new != normalize_doi_value(entry.get("doi")):
                actions.append(self._make_action(entry, "doi", new=doi_new, confidence=conf, source=best.get("source", "candidate"), reason="High-confidence candidate DOI"))
        return actions
